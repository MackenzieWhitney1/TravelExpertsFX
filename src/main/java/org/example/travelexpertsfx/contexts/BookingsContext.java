...
import org.example.travelexpertsfx.controllers.BookingAuditController;
...
public class BookingsContext implements ITableContext {
    ...
    public int getSelectedInfoId() {
        ...
        return data.get(index).getBookingId();
    }
    public void selectInfo(int selected) {
        bookingTable.getSelectionModel().select(selected);
    }
    @Override
    public void audit(){
        FXMLLoader fxmlLoader = new FXMLLoader(TravelExpertsApplication.class.getResource("booking-audit-view.fxml"));
        Scene scene = null;
        try{
            scene = new Scene(fxmlLoader.load());
        }catch (IOException e){
        ...
        BookingAuditController dialogController = fxmlLoader.getController();
        Stage stage = new Stage();
        stage.setScene(scene);
        stage.showAndWait();
        displayTableContent();
